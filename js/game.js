...
// The main module supplies these two functions from the Firebase layer.
let applySaveData = null;

export function setSaveHandler(handler) {
    applySaveData = handler;
}
...
export function getCurrentGameState() {
    return {
        ...
        levelResults: structuredClone(levelResults)
    };
}
...
let deaths = 0;
const respawnPoint = { x: 120, y: 300 };
...
let fireflies = [];
let checkpoints = [];
const goal = { x: 0, y: 0, width: 0, height: 0 };
// PLAYER
// --------------------------------------------------
...
async function saveLevelResult() {
    const existing = levelResults[currentLevel];
    ...
    if (
        !existing ||
        collectedFireflies > existing.fireflies ||
        (collectedFireflies === existing.fireflies && deaths < existing.deaths) ||
        (collectedFireflies === existing.fireflies && deaths === existing.deaths && completionTime < existing.time)
    ) {
